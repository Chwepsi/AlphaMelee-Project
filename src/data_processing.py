...
def get_frame_data(game: Game, frame_number: int) -> Optional[Dict[str, Any]]:
    """Extrait les données d'une frame spécifique en évitant les cas critiques et les erreurs"""
    try:
        frame = game.frames[frame_number] # Essaye d'accéder à la frame
    except IndexError:
    ...
    for port, port_data in enumerate(game.frames[frame_number].ports):  # Parcourt les ports et leurs informations
        ...
        if frame_number > 0:  # Vérifie qu'il y a une frame précédente
            previous_frame = game.frames[frame_number - 1]
        ...
        data['players'][port] = {
            ...
            'state': player.post.state.name,  # État actuel (e.g., ENTRY, ATTACK)
